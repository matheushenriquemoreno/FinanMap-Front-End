  ...
  <div class="text-start">
    ...
    <div class="flex justify-center items-center q-gutter-sm gap-1">
      <div style="width: 45%; min-width: 220px">
        <InputSelectMes label="Mês" v-model:model-value="mes" :styled="styled" />
      </div>

      <div style="width: 45%; min-width: 220px">
        <InputSelectAno v-bind="attrs" label="Ano" v-model:model-value="ano" :styled="styled" />
      </div>
    </div>
  </div>
</template>
...
const mes = ref(props.modelValue.mes);
const ano = ref(props.modelValue.ano);

// Definindo emits
const emit = defineEmits(['update:modelValue']);

// Observar mudanças e emitir eventos
watch([mes, ano], ([mes, ano]) => {
  modelLocal.value = { mes, ano };
});
