<template>
  ...
  >
    <q-card
      :style="
        $q.screen.gt.xs
          ? 'min-width: 480px; max-width: 600px; border-radius: 16px;'
          : 'width: 100%; height: 100%; max-height: 100vh; border-radius: 0; display: flex; flex-direction: column;'
      "
    >
      <q-card-section class="q-pb-none">
        <div class="row items-center justify-between">
          ...
          <q-btn icon="close" flat round dense v-close-popup aria-label="Fechar detalhes da meta" />
        </div>
      </q-card-section>
        ...
        <div class="detalhes-resumo q-mb-md">
          ...
          <div class="row justify-between q-mt-xs">
            <span class="text-caption text-grey-6"
              >{{ meta.percentualProgresso?.toFixed(1) || 0 }}%</span
            >
            <span class="text-caption text-grey-6">
      ...
      <template v-if="meta && meta.contribuicoes && meta.contribuicoes.length > 0">
        ...
        <q-card-section class="col" style="display: flex; flex-direction: column; min-height: 0">
          ...
          <q-list separator class="historico-list col">
            <q-item
              v-for="contribuicao in meta.contribuicoes"
              :key="contribuicao.id"
              class="q-py-md q-px-sm"
            >
              <q-item-section avatar top v-if="$q.screen.gt.xs">
                <q-avatar
                  :color="contribuicao.origem === 'Investimento' ? 'primary' : 'positive'"
                  text-color="white"
                  size="42px"
                >
                  <q-icon
              ...
              <q-item-section>
                ...
                <q-item-label v-if="contribuicao.origem === 'Investimento'" class="q-mt-sm">
                  ...
                  >
                    ...
                    <span
                      >Investimento -
                      <span class="text-bold">{{ contribuicao.nomeInvestimento }}</span></span
                    >
                  </q-badge>
                </q-item-label>
              </q-item-section>
...
const categoriaConfig = computed(() => {
  ...
  return (
    CATEGORIA_META_CONFIG[props.meta.categoria] || CATEGORIA_META_CONFIG[CategoriaIconeMeta.Outro]
  );
});
